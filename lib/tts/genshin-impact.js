...
export function genshinRvc(options = {}) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!useTTS && Buffer.isBuffer(audio_buffer)) {
        ...
        try {
            const uploadResponse = await axios.post(`${BASEURL}/upload`, form, { headers: { ...form.getHeaders(), 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36' } });
